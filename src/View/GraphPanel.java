...
public class GraphPanel extends JPanel {
    ...
    public GraphPanel() {
        ...
        for(double i = 10, j = 10; i <= 60; i = i+5) {
    ...
    protected void paintComponent(Graphics g) {
        ...
        double xScale = ((double) getWidth() - (2 * padding) - labelPadding) / (temperature.size() - 1);
        double yScale = ((double) getHeight() - 2 * padding - labelPadding) / (getMaxScore() - getMinScore());

        List<Point> graphPoints = new ArrayList<>();
        for (int i = 0; i < temperature.size(); i++) {
            int x1 = (int) (i * xScale + padding + labelPadding);
            int y1 = (int) ((getMaxScore() - temperature.get(i)) * yScale + padding);
        ...
        for (int i = 0; i < 61; i++) {
            if (temperature.size() > 1) {
                int x0 = i * (getWidth() - padding * 2 - labelPadding) / (60) + padding + labelPadding;
                int x1 = x0;
...
//    	JFrame a = new JFrame();
//    	a.setMinimumSize(new Dimension(800,600));
//    	a.add(new GraphPanel());
